...
For call=observe with an image, ignore the character and dialogue tasks. present must contain each visible exact label from prior_canonical once and no other labels. known may contain at most one fresh detailed record for each present prior object, and every known canonical must also occur in present. new must contain each salient visible object whose canonical is not in prior_canonical once; never repeat a prior canonical in new even when its descriptive label or attributes changed. Each known and new object must use {"label":string,"canonical":string,"attributes":[strings],"bbox_norm":[x,y,width,height]}. Report visible colors, materials, markings, object type, and the attribute "held in hand" when applicable. bbox_norm uses decimal x, y, width, and height: width and height must be greater than zero, x+width must be at most 1, and y+height must be at most 1. If prior_canonical is empty, present and known must be empty and discovered objects go in new. Return exactly present, known, and new; never return say, plan, dialogue, or prose.
